          ...
          .map((tk) =>
            timing.CustomTimings[tk]
              ...
              <div class="mp-nested-timing">
                  <span class="mp-duration">${timing.CustomTimingStats[tk].Count}</span> ${encode(tk)} call${
...
<tr class="${timing.IsTrivial ? 'mp-trivial' : ''}${timing.DebugInfo ? ' mp-debug' : ''}" data-timing-id="${timing.Id}">
  ...
  <td class="mp-duration">
    <a class="mp-queries-show${timing.HasWarnings[tk] ? ' mp-queries-warning' : ''}" title="${duration(
            timing.CustomTimingStats[tk].Duration
          )} ms in ${timing.CustomTimingStats[tk].Count} ${encode(tk)} call(s)${
            timing.HasDuplicateCustomTimings[tk] ? '; duplicate calls detected!' : ''
          }">
      ${duration(timing.CustomTimingStats[tk].Duration)}
      (${timing.CustomTimingStats[tk].Count}${
            timing.HasDuplicateCustomTimings[tk] || timing.HasWarnings[tk] ? '<span class="mp-warning">!</span>' : ''
          })
    </a>
  </td>`
        : '<td></td>'
      ...
      <table class="mp-timings mp-client-timings">
        ...
        <tbody>
          ...
          <tr class="${t.isTrivial ? 'mp-trivial' : ''}">
            ...
            <td class="t-${t.type}${t.point ? ' t-point' : ''}"><div style="margin-left: ${t.left}; width: ${
                t.width
              };"></div></td>
            <td class="mp-duration">
