import { ClientTiming, ITimingInfo } from "./";

export class ResultRequest {
  public Id: string;
  public Performance!: ClientTiming[];
  public Probes!: (ClientTiming | null)[];
  ...
  constructor(id: string, perfTimings: ITimingInfo[]) {
    ...
    if (perfTimings && window.performance && window.performance.timing) {
      ...
      this.Performance = perfTimings
        .filter((current) => resource[current.name as keyof PerformanceTiming])
        .map((current, i) => ({ item: current, index: i }))
        .sort((a, b) => (resource[a.item.name as keyof PerformanceTiming] as number) - (resource[b.item.name as keyof PerformanceTiming] as number) || a.index - b.index)
        .map((x, i, sorted) => {
          ...
          return {
            ...current,
            ...{
              startTime: (resource[current.name as keyof PerformanceTiming] as number) - start,
              timeTaken: !next ? 0 : (resource[next.name as keyof PerformanceTiming] as number) - (resource[current.name as keyof PerformanceTiming] as number),
            },
          };
        })
        .map((item, i) => ({
          ...
          Duration: item.point ? 0 : item.timeTaken,
        }));
